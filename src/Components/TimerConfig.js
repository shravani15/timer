...
class TimerConfig extends Component {
    ...
    handleChange(ev) {
        ...
        if(ev.target.id === 'minutes') newBaseTime.subtract(newBaseTime.get('minute'), 'minutes').add(parseInt(ev.target.value, 10), 'minutes');
        if(ev.target.id === 'seconds') newBaseTime.subtract(newBaseTime.get('second'), 'seconds').add(parseInt(ev.target.value, 10), 'seconds');
